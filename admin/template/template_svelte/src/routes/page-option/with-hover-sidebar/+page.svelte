<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Hover Sidebar');
    
		let response = await fetch('/data/page-option/with-hover-sidebar-code-1.json');
		code1 = (await response.text());
		
		$appOptions.appSidebarHover = true;
  });
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with hover sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
