<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Two Sidebar');
    
		let response = await fetch('/data/page-option/with-two-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with two sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
