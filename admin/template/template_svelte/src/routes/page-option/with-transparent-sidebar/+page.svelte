<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Transparent Sidebar');
    
		let response = await fetch('/data/page-option/with-transparent-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with transparent sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
