<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Minified Sidebar');
    
		let response = await fetch('/data/page-option/with-minified-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with minified sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
