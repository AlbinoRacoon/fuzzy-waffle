<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Search Sidebar');
    
		let response = await fetch('/data/page-option/with-search-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with search sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
