<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Wide Sidebar');
    
		let response = await fetch('/data/page-option/with-wide-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with wide sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
