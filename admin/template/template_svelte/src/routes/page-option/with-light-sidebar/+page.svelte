<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Light Sidebar');
    
		let response = await fetch('/data/page-option/with-light-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with light sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
