<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Boxed Layout');
    
		let response = await fetch('/data/page-option/with-boxed-layout-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with boxed layout setting.
		</p>
	</PanelBody>
	{#if code1}
