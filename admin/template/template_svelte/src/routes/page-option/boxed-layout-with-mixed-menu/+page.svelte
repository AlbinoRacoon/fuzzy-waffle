<script>
  ...
  onMount(async () => {
    setPageTitle('Boxed Layout with Mixed Menu');
    
		let response = await fetch('/data/page-option/boxed-layout-with-mixed-menu-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for boxed layout with mixed menu setting.
		</p>
	</PanelBody>
	{#if code1}
