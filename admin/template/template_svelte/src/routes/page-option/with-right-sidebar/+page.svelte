<script>
  ...
  onMount(async () => {
    setPageTitle('Page with Right Sidebar');
    
		let response = await fetch('/data/page-option/with-right-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page with right sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
