<script>
  ...
  onMount(async () => {
    setPageTitle('Page without Sidebar');
    
		let response = await fetch('/data/page-option/without-sidebar-code-1.json');
...
<Panel class="panel-inverse">
	...
	<PanelBody>
		<p>
			Add the following code for page without sidebar setting.
		</p>
	</PanelBody>
	{#if code1}
...
<p>
	<a href="#/" on:click|preventDefault={() => history.back()} class="btn btn-success">
