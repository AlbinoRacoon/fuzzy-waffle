			...
			if (elms4) {
				elms4.map(function(elm) {
					...
					if (targetText.search(targetValue) > -1) {
						...
						if (targetElm2) {
							var targetElm3 = targetElm2.querySelector('.menu-submenu .menu-item.d-none');
							if (targetElm3) {
						...
						if (targetElm4) {
							...
							var targetElm5 = targetElm4.querySelector('.menu-item:not(.has-text)');
							if (targetElm5) {
							...
							if (targetElm6) {
								...
								var targetElm7 = targetElm6.closest('.has-sub:not(.has-text)');
								if (targetElm7) {
				...
				elm.onmouseenter = function() {
					...
					if (appElm && appElm.classList.contains('app-sidebar-minified')) {
						...
						if (targetMenuHtml) {
							...
							var targetHeight2 = document.querySelector('#app-sidebar-float-submenu').clientHeight;
							var floatSubmenuElm2 = document.querySelector('#app-sidebar-float-submenu');
							...
							if ((windowHeight - targetTop) > targetHeight2) {
								if (floatSubmenuElm2) {
									...
									floatSubmenuElm2.style.left = targetLeft + 'px';
									floatSubmenuElm2.style.bottom = 'auto';
									floatSubmenuElm2.style.right = targetRight + 'px';
								}
		...
		<div class="app-sidebar-content">
			...
			<div class="menu">
				...
				<div class="menu-item d-flex">
					<a href="#/" aria-label="Link" class="app-sidebar-minify-btn ms-auto d-flex align-items-center text-decoration-none" on:click={minifiedToggler}><i class="fa fa-angle-double-left"></i></a>
				</div>
				<!-- END minify-button -->
			</div>
			<!-- END menu -->
...
<div class="app-sidebar-mobile-backdrop"><a href="#/" aria-label="Link" on:click={mobileToggler} class="stretched-link">&nbsp;</a></div>