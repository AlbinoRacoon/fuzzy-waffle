...
Website: http://www.seantheme.com/color-admin/
	...
	04. Handle Payment Type Selection
	05. Handle Checkout Qty Control
	06. Handle Product Image
	07. Handle Paroller
	08. Handle Check Bootstrap Version
	09. Handle Get Css Variable
	10. Handle Navbar Transparent
...
var app = {
	...
	themePanel: {
		...
		themeRtlMode: {
			toggleAttr: 'name="app-theme-rtl-mode"',
			cookieName: 'app-theme-rtl-mode'
		}
	},
	font: {},
	color: {},
...
/* 10. Handle Navbar Transparent
------------------------------------------------ */
var handleNavbarTransparent = function() {
	var checkScroll = function() {
		var target = document.querySelector('#header');
		if (!target) return;

		const isTransparent = target.classList.contains('navbar-transparent');

		if (window.scrollY > 100) {
			if (isTransparent) {
				target.classList.add('bg-inverse');
				target.setAttribute('data-bs-theme', 'dark');
			}
		} else {
			if (isTransparent) {
				target.classList.remove('bg-inverse');
